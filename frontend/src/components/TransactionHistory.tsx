...
const TableHeader = () => (
  <thead>
    <tr>
      {["Donor", "Amount", "Cumulative", "Timestamp", "Transaction"].map(
        (header, index) => (
          <th
            key={header}
...
export function TransactionHistory({ deposits, loading }: TransactionHistoryProps) {
  if (loading) {
      ...
      <table className="min-w-full">
        ...
        <tbody>
          {deposits.map((deposit) => (
            <tr key={deposit.id} className="border-t border-gray-100">
              <td className="py-3">
                <Identity
                  address={deposit.donor.address as `0x${string}`}
                  chain={base}
                ...
                >
                  ...
                  />
                  <span>{shortenAddress(deposit.donor.address)}</span>
                </Identity>
              </td>
              <td className="py-3">
                ${(Number(deposit.amount) / 10**6).toLocaleString('en-US', {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2
                })}
              </td>
              <td className="py-3 hidden md:table-cell">
                ${(Number(deposit.cumulativeAmount) / 10**6).toLocaleString('en-US', {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2
                })}
              </td>
              <td className="py-3 hidden md:table-cell">
                {formatDate(new Date(Number(deposit.timestamp) * 1000))}
              </td>
              <td className="py-3 hidden md:table-cell">
                <Link
